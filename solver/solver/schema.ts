...
export const createSolverSchema = (graph: SolverGraph): SolverSchema => {
          ...
          ...Object.fromEntries(
            edgeTypes.map((edgeType) => {
              const connection = connections.find(
                (connection) => connection.name === `${edgeType.name}Connection`,
              );
              if (!connection) throw new Error(`connection not found: ${edgeType.name}`);
              return [
  ...
  const connections = graph.edgeTypes.map((edgeType) => {
    return new GraphQLObjectType({
      ...
      fields: () => {
        const edge = edges.find((edge) => edge.name === `${edgeType.name}Edge`);
        if (!edge) throw new Error(`edge not found: ${edgeType.name}`);
        return {
          edges: {
